...
import type { FormItemContentRenderParams, FormItemRenderOptions, VXETableCore, VxeColumnPropTypes, VxeGlobalInterceptorHandles, VxeGlobalRendererHandles, VxeTableDefines } from 'vxe-table'
import type { Dayjs, OpUnitType } from 'dayjs'
import type { RangePickerDateProps } from 'ant-design-vue/es/vc-picker/RangePicker'
...
function getOns(renderOpts: VxeGlobalRendererHandles.RenderOptions, params: VxeGlobalRendererHandles.RenderParams, inputFunc?: Fn, changeFunc?: Fn) {
  const { events } = renderOpts
  const modelEvent = getModelEvent(renderOpts)
  const changeEvent = getChangeEvent(renderOpts)
  const isSameEvent = changeEvent === modelEvent
  const ons: { [type: string]: Fn } = {}
  XEUtils.objectEach(events, (func: Fn, key: string) => {
...
function getFilterOns(renderOpts: VxeGlobalRendererHandles.RenderOptions, params: VxeGlobalRendererHandles.RenderFilterParams, option: VxeTableDefines.FilterOption, changeFunc: Fn) {
  return getOns(renderOpts, params, (value: any) => {
    // 处理 model 值双向绑定
    option.data = value
  }, changeFunc)
...
export function createFilterRender(defaultProps?: { [key: string]: any }, component?: unknown) {
  return function (renderOpts: VxeColumnPropTypes.FilterRender & { name: string }, params: VxeGlobalRendererHandles.RenderFilterParams) {
    ...
    return [
      h('div', {
        ...
        return h(component ?? resolveComponent(name), {
          key: oIndex,
          ...attrs,
...
function createExportMethod(getExportCellValue: Fn) {
  return function (params: VxeGlobalRendererHandles.ExportMethodParams) {
    const { row, column, options } = params
 ...
 * 获取日期对比的深度
 */
export function getDateFilterDepth(props?: RangePickerDateProps<Dayjs>): OpUnitType {
  if (!props)
    return 'D'
  switch (true) {
    case props.showTime:
      return 's'
    case props.picker === 'year':
      return 'y'
    case props.picker === 'month':
      return 'M'
    default:
      return 'D'
  }
  ...
  install(vxetablecore: VXETableCore) {
    ...
    renderer.mixin({
      ...
      ARangePicker: {
        ...
        // 允许部分为空
        renderFilter: createFilterRender({ allowEmpty: [true, true] }),
        renderItemContent: createFormItemRender(),
        defaultFilterMethod(params) {
          const cellValue = params.cellValue
          const [start, end] = params.option.data as [Dayjs, Dayjs]

          const depth = getDateFilterDepth(params.column.filterRender.props)

          return !(start?.isAfter(cellValue, depth) || end?.isBefore(cellValue, depth))
        },
